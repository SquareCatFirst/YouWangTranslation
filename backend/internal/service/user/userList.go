...
func UserList(c *gin.Context) {
	...
	if err1 != nil || err2 != nil || page <= 0 || pageSize <= 0 {
		util.SendJSON(c, -1, "分页参数错误", []interface{}{}, 0, "/api/v1/auth/user/list", "GET")
		return
	}
	...
	if err := config.DB.Model(&model.User{}).Count(&total).Error; err != nil {
		util.SendJSON(c, -1, "获取用户总数失败", []interface{}{}, 0, "/api/v1/auth/user/list", "GET")
		return
	}
	if total == 0 {
		util.SendJSON(c, 0, "获取成功", []interface{}{}, 0, "/api/v1/auth/user/list", "GET")
		return
	}
	...
	if err := config.DB.
		...
		util.SendJSON(c, -1, "获取用户列表失败", []interface{}{}, 0, "/api/v1/auth/user/list", "GET")
		return
	}
	...
	util.SendJSON(c, 0, "获取成功", list, int(total), "/api/v1/auth/user/list", "GET")
}
